...
if configs['task'].lower() == 'classification':
    model_dict = {'lr': LogisticRegression(n_jobs=-1, solver='saga'),
...
if 'pathway_file' in configs:
    ...
    data_input = data_input[
        [x for x in data_input.columns if (x.split("_")[0] in cancer_genes) or (x.split("_")[0] == 'tissue')]]
    ...
    for cell_lines_train_index, cell_lines_val_index in cv.split(cell_lines_all):
        ...
        for i in trange(num_targets):
            ...
            if configs['task'].lower() == 'classification':
                ...
                score_dict = {'target': merged_df_train.columns[i], 'run': f"cv_{count}", 'auc': val_auc,
                              'acc': val_acc}
            else:
                ...
                score_dict = {'drug_id': merged_df_train.columns[i], 'run': f"cv_{count}",
