    ...
    for n in range(NUM_REPATE):
        ...
        for cell_lines_train_index, cell_lines_val_index in tqdm(
            ...
            merged_df_train = pd.merge(
                input_df[input_df.index.isin(train_lines)],
                target_df,
                on=['Cell_line'])
            ...
            if clf_name == "RF":
                ...
                raise Exception
            clf.fit(merged_df_train.iloc[:, :num_of_features],
...
input_df = pd.read_csv(
    "../data/processed/omics/tcga_brca_mutation_cnv_rna_log2.csv.gz",
    index_col=0)

genes = np.unique(([x.split("_")[0] for x in input_df.columns if x.split("_")[0] != 'tissue']))
...
class_name_to_id = dict(
    zip(sorted(target_df.iloc[:, 0].unique()),
        list(range(target_df.iloc[:, 0].unique().size))))
id_to_class_name = dict(
    zip(list(range(target_df.iloc[:, 0].unique().size)), sorted(target_df.iloc[:, 0].unique())))
...
lr_results_df.to_csv("../results/tcga_brca_subtype/lr_mutation_cnv_rna_allgenes.csv",
                     index=False)
print("Running RF")
rf_results_df = run_model(input_df, "RF")
rf_results_df.to_csv("../results/tcga_brca_subtype/rf_mutation_cnv_rna_allgenes.csv",
                     index=False)
print("Running KNN")
knn_results_df = run_model(input_df, "KNN")
knn_results_df.to_csv("../results/tcga_brca_subtype/knn_mutation_cnv_rna_allgenes.csv",
                     index=False)
print("Running DR")
dt_results_df = run_model(input_df, "DT")
dt_results_df.to_csv("../results/tcga_brca_subtype/dt_mutation_cnv_rna_allgenes.csv",
                     index=False)
print("Running PCA")
pca_input_df = pd.read_csv(
    "../data/DR/pca/tcga_brca_mutation_cnv_rna_allgenes.csv",
    index_col=0)
rf_pca_results_df = run_model(pca_input_df, "RF", data_type = ['DR'])
rf_pca_results_df.to_csv("../results/tcga_brca_subtype/pca_rf_mutation_cnv_rna_allgenes.csv",
                     index=False)
print("Running moCluster")
moCluster_input_df = pd.read_csv(
    "../data/DR/moCluster/tcga_brca_mutation_cnv_rna.csv",
    index_col=0)
moCluster_results_df = run_model(moCluster_input_df, "RF", data_type = ['DR'])
moCluster_results_df.to_csv("../results/tcga_brca_subtype/moCluster_rf_mutation_cnv_rna_allgenes.csv",
                     index=False)